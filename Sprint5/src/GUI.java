...
public class GUI extends JFrame {
    ...
    public GUI(){
        ...
        recorder.start(game, blueIsComputer, redIsComputer);
    ...
    private class BoardPanel extends JPanel {
            ...
            addMouseListener(new MouseAdapter() {
                ...
                public void mouseClicked(MouseEvent e) {
                    ...
                    if (game.getGameBoard().placeLetter(row, col, letter)) {
                        // Record Move
                        recorder.recordMove(row, col, letter, game.getCurrentPlayer());
                        ...
                        if (game instanceof SimpleGame){
                            ...
                            if (simGame.isGameOver()){
                                handleGameEnd(simGame.getWinner(), null);
                                return;
                            }
                        ...
                        // Within the SOS game on General Mode
                            ...
                            if (genGame.isGameOver()){
                                handleGameEnd(genGame.getWinner(), null);
                                return;
                            }
                        }
                        else{
                            if (!SOSFound) game.nextPlayer();
                            else game.SOSPlacementFoundCount(row, col, letter);
                        }
        ...
        protected void paintComponent(Graphics g) {
            ...
            for (int i = 1; i < size; i++) {
                g.drawLine(i * width / size, 0, i * width / size, height);
            }

            for (int i = 1; i < size; i++) {
                g.drawLine(0, i * height / size, width, i * height / size);
            }
                ...
                for (int j = 0; j < size; j++){
                    ...
                    if (letter != ' '){
                        g.drawString(String.valueOf(letter),
                                j * width / size + width / size / 2 - 5,
                                i * height / size + height / size / 2 + 5);
                    }
                }
            }
        }
    ...
    private void processComputerTurn(){
        ...
        while (!game.isGameOver() && ((game.getCurrentPlayer() == Player.BLUE && blueIsComputer)
            ...
            recorder.recordMove(r, c, letter, game.getCurrentPlayer());
            ...
            // In the instance of a General Game
                ...
                // display winner
                if (genGame.isGameOver()){
                    handleGameEnd(genGame.getWinner(), genGame.getScore());
                    return;
                }
            }
    ...
    // Handles the ending of a game session
    private void handleGameEnd(Player winner, String score) {
        recorder.end();

        // Choose and name new file for the record
        JFileChooser file = new JFileChooser();
        if (file.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
            try {
                recorder.saveToFile(file.getSelectedFile());
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        }
        ...
        String message;
        if (score == null)
            message = (winner != null) ? "Winner: " + winner : "Draw!";
        else
            message = (winner != null ? "Winner: " + winner : "Draw!") + "\n" + score;
    ...
    // Load in the replay file
    private void loadReplay() {
        // Choose the saved file
        JFileChooser savedfile = new JFileChooser();
        if (savedfile.showOpenDialog(this) != JFileChooser.APPROVE_OPTION)
            return;

        try {
            GameReplayer replay = new GameReplayer();
            replay.load(savedfile.getSelectedFile());

            // Set up the game options (Mode & Size)
            game = replay.mode.equals("SIMPLE") ?
                    new SimpleGame(replay.size) :
                    new GeneralGame(replay.size);


            boardSize = replay.size;
            blueIsComputer = replay.blueComputer;
            redIsComputer = replay.redComputer;

            if (replay.startPlayer != null) {
                game.setCurrentPlayer(replay.startPlayer);
            }

            boardPanel.repaint();
            updateTurn();

            // Progress through the saved game file
            new Thread(() -> {
                try {
                    Thread.sleep(1000);
                    for (GameReplayer.Move m : replay.moves) {
                        // Thread.sleep(500);
                        Thread.sleep(1000);
                        replay.applyMove(game, m);
            ...
            JOptionPane.showMessageDialog(this, "Replay failed to load.");
        }
    }
